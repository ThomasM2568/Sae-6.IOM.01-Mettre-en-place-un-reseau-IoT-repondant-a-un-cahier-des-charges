...
if docker ps -a --format '{{.Names}}' | grep -q "^$container_name$"; then
    ...
    docker run -d --name mariadb-docker -e MYSQL_ROOT_PASSWORD=tpRT9025 -p 3306:3306 -v mariadb_save:/var/lib/mysql mariadb 
    ...
    docker exec -it mariadb-docker bash -c "echo 'CREATE DATABASE ecomddb;' | mysql -uroot -p'tpRT9025'"
    docker exec -i mariadb-docker mysql -uroot -p"tpRT9025" -D ecomddb < ddb.sql
